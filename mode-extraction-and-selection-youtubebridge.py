...
avi_fn = 'data/tree-and-clouds.MOV'
print('Reading AVI:', avi_fn)

frames = vvt.motion.read_frames(avi_fn)
frames = [cv2.resize(frame, (frames[0].shape[1]//3, frames[0].shape[0] // 3)) for frame in frames] # comment these out if not for self captured data
frames = [cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE) for frame in frames] # comment these out if not for self captured data
