...
modal_data_file = 'data/%s/modal_observations2.pkl' % video
...
force_application_x_range =  [50,110]
force_application_y_range =  [440,570]

# for center block
# force_application_x_range = [115,165]
# force_application_y_range = [260,375]

# for left most block
# force_application_x_range = [175,215]
# force_application_y_range = [90, 185]
...
def read_ref_frame(vfile='data/resonance-test.mp4', start_frame=750) :
...
def calc_fi(mi_dx, mi_dy, dir, alpha=2, cur_x=None, cur_y=None) :
...
def run_simulation_per_pixel(Qi, mi_dx, mi_dy, wi, h=0.04, dir=np.array([0,-1]), mass_i=1, timesteps=1, type_manip=0, cur_x=None, cur_y=None) : 
...
def run_simulation_over_all_pixels_per_mode(Q, m_dx, m_dy, w, timesteps=4) :
...
for i in range(len(freqs)) : 
    new_Q[:,i] = run_simulation_over_all_pixels_per_mode(Q[:,i].copy(), mdx[i].copy(), mdy[i].copy(), freqs[i])
    print('modal {} done'.format(i))
...
save_im_vvt_style(D_x_filtered, 'D_x_sigma50_maniptype0_targetted_ts5.png')
save_im_vvt_style(D_y_filtered, 'D_y_sigma50_maniptype0_targetted_ts5.png')
save_im(new_im, 'new_im_sigma50_maniptype0_targetted_ts5.png')

D_x_filtered = gaussian_filter(D_x_nonoutlier, 160)
D_y_filtered = gaussian_filter(D_y_nonoutlier, 160)

new_im = create_displaced_image(D_x_filtered, D_y_filtered, ref_frame)

save_im_vvt_style(D_x_filtered, 'D_x_sigma160_maniptype0_targetted_ts5.png')
save_im_vvt_style(D_y_filtered, 'D_y_sigma160_maniptype0_targetted_ts5.png')
save_im(new_im, 'new_im_sigma160_maniptype0_targetted_ts5.png')
